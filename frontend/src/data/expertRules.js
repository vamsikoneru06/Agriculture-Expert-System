/**
 * Knowledge Base – Agriculture Expert System
 * 25 IF-THEN rules covering soil × weather × season combinations.
 * Each rule maps input conditions to crop recommendations, fertilizers,
 * pest control advice, water requirements, expected yield, and tips.
 */
...
export const expertRules = [
  /* ── 1 ── */
  {
    ...
    output: {
      ...
      pestControl: 'Neem-based spray to prevent downy mildew',
      waterRequirement: 'Low – 400-500 mm per season',
      expectedYield: '1.5 – 2.5 tons/hectare',
      tips: 'Plant at 45 cm × 15 cm spacing. Highly drought-tolerant.',
      confidence: 95,
    },
  },
  /* ── 2 ── */
  ...
  /* ── 9 ── */
  {
    ...
    output: {
      ...
      tips: "Black soil's high water-holding capacity reduces irrigation needs.",
      confidence: 92,
    },
  },
  /* ── 10 ── */
  ...
  /* ── 17 ── */
  {
    ...
    output: {
      ...
      pestControl: 'Red spider mite: Abamectin 0.5 ml/L; Downy mildew: Ridomil',
  ...
  /* ── 19 ── */
  {
    ...
    output: {
      ...
      pestControl: 'Stem borer: Endosulfan spray; Shoot fly: Carbofuran granules',
  ...
  /* ── 21 ── */
  {
    ...
    output: {
      ...
      waterRequirement: 'Moderate – 350-550 mm; stop 10 days before harvest',
...
/**
 * Match rules against user-provided conditions.
 * Returns the single best-matching rule output, or null if no match.
 */
export function matchRules(conditions) {
  const { soilType, weather, season } = conditions;

  const match = expertRules.find(
    (rule) =>
      rule.conditions.soilType === soilType &&
      rule.conditions.weather === weather &&
...
/* Drop-down option lists used in ExpertSystem form */
export const soilOptions = [
  { value: 'sandy', label: 'Sandy Soil' },
  { value: 'clay',  label: 'Clay Soil' },
  { value: 'loamy', label: 'Loamy Soil' },
  { value: 'black', label: 'Black (Regur) Soil' },
